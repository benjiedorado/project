			...
			<option value="all"  <?php echo isset($position_id) && $position_id == "all" ? "selected" : '' ?>>All</option>
				<?php foreach($pos as $k => $v): ?>
					<option value="<?php echo $k ?>" <?php echo isset($position_id) && $position_id == $k ? "selected" : '' ?>><?php echo $v ?></option>
								...
								$i = 1;
								$stats = $conn->query("SELECT * FROM recruitment_status where status=1 order by id asc");
								$stat_arr[0] = "New";
...
<select name="" class="custom-select browser-default select2" id="filter_statusall">
<div class="row">
			<div class="col-md-12 form-group">
		<?php foreach ($stat_arr as $key => $value): ?>		
		<option value="<?php echo $key ?>" <?php echo isset($pid) && $pid == $key ? "selected" : '' ?>><?php echo $value ?></option>
         <?php endforeach; ?>
...
$('#filter_statusall').change(function(){
	location.href = "index.php?page=applications&pid="+$(this).val()+'&position_id=<?php echo $position_id ?>'
})
